...
import type { AdminUseCases } from '../application/admin/AdminUseCases'
import type { AuthUseCases } from '../application/auth/AuthUseCases'
import type { AuthorizeContextService } from '../application/auth/AuthorizeContextService'
import type {
  AdminOverview,
  AdminTab,
  ApplicationDetails,
...
function AuthPage({
...
}: Pick<AppDependencies, 'authUseCases' | 'authorizeContextService'>) {
  const authorizeContext = useMemo(
    () => authorizeContextService.fromCurrentUrl(),
    [authorizeContextService],
  )
  ...
  async function handleRegister() {
    await authUseCases.registerUser({ name, email, password })
  ...
  async function handleLogin() {
    ...
    const data = await authUseCases.authorizeUserLogin({
      email,
      password,
      authorizeContext,
    })
...
>) {
  ...
  const [newApplication, setNewApplication] = useState<RegisterApplicationInput>({
    name: '',
    clientId: '',
    audience: '',
    clientSecret: '',
  ...
  const [error, setError] = useState('')
  const [status, setStatus] = useState('')
  const [adminToken, setAdminToken] = useState(() => adminTokenStorage.get())
  ...
  async function loadAdminData(nextApplicationId = selectedApplicationId) {
    ...
    try {
      const workspace = await adminUseCases.loadWorkspace(nextApplicationId)

      setOverview(workspace.overview)
      setSelectedApplicationId(workspace.selectedApplicationId)
      setDetails(workspace.details)
    } catch (requestError) {
  ...
  async function selectApplication(applicationId: string) {
    ...
    setDetails(await adminUseCases.selectApplication(applicationId))
  }
  ...
  async function saveApplication(event: FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      await adminUseCases.registerApplication(newApplication)
      setNewApplication({
        name: '',
        clientId: '',
        audience: '',
  ...
  async function saveRole(event: FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      await adminUseCases.registerRole({
  ...
  async function renameRole(roleId: number) {
    await runAction(async () => {
      await adminUseCases.renameRole({ roleId, name: editingRoleName })
      setEditingRoleId(null)
      setEditingRoleName('')
    }, 'Role atualizada.')
  }
  ...
  async function deleteRole(roleId: number) {
    await runAction(async () => {
      await adminUseCases.deleteRole(roleId)
    }, 'Role removida.')
  }
  ...
  async function saveAssignment(event: FormEvent<HTMLFormElement>) {
    ...
    await runAction(async () => {
      await adminUseCases.assignRole({
        applicationClientId: Number(selectedApplicationId),
  ...
  async function deleteAssignment(assignmentId: number) {
    await runAction(async () => {
      await adminUseCases.deleteAssignment(assignmentId)
    }, 'Atribuicao removida.')
  }
  ...
  return (
    <main className="admin-shell admin-shell-modern">
      ...
      <section className="admin-content">
        ...
        {tab === 'docs' ? (
          <DocsTab docsAuthApiUrl={docsAuthApiUrl} />
        ) : !adminToken ? (
          <AdminLogin
...
function AdminLogin({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await authUseCases.loginAdmin({
        email,
        password,
...
function DocsTab({ docsAuthApiUrl }: { docsAuthApiUrl: string }) {
